...
class QuantumComputerSimulator:
    ...
    def generate_frame(self, phase, highlight_pair=None):
        ...
        empty_columns = (os.get_terminal_size().columns - len(out_frame[0]))
        horizontal_padding = " "*(empty_columns//2)
        for i in range(len(out_frame)):
            out_frame[i] = horizontal_padding+out_frame[i]

        # Add vertical padding
        empty_lines = (os.get_terminal_size().lines - len(out_frame))
        vertical_padding = [" " for i in range(empty_lines//2)]
        out_frame = vertical_padding+out_frame
